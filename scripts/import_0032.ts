...
const CSV_PATH = path.join(__dirname, '../bulkUpdate/trans-0032.csv');
...
const importTransactions = async () => {
    ...
    try {
        ...
        // Fetch a valid user_id for context (or use one from CSV if valid)
        // CSV has user_id, let's check if it exists? 
        // actually CSV user_id: 5e189da8-2f10-43a6-bb19-be0b05f098ba
        // Let's rely on the CSV user_id if valid, or fallback.
        // But to be safe, we can just use the user_id from the CSV.
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // 1. Parse Date (DD-MM-YY H:MM)
            const parseDate = (d: string) => {
                if (!d) return new Date().toISOString();
                // Expect 06-01-25 0:00
